const Card = ({
  ...
  return (
    <div className="border-2 border-white/20 bg-white/10 backdrop-blur-md rounded-lg text-chalk p-4 shadow-lg shadow-white-grey/10">
      <div className="">
        {isCompleted && (
          <img
            src={projectImage}
      ...
      <div className="my-6 space-y-5">
        <div className="flex items-center justify-between">
          <h2 className="font-nunito tracking-wider font-semibold text-base">
            {projectName}
          </h2>
          <h3 className=" font-roboto-mono tracking-wide text-xs text-white-grey p-1.5 rounded-lg border-2 border-chalk/40 ">
            {stackType}
          </h3>
        </div>
        ...
        <ul className="flex items-center flex-wrap gap-x-2 gap-y-3">
          {techStack.map((tech) => (
            <li
              key={tech.name}
              className="flex items-center gap-2 p-2 rounded-lg border-2 border-chalk/40"
            >
              <img src={tech.src} alt={tech.name} className="h-5 w-5 " />
              <p className="text-xs border-l-2 border-chalk pl-2">
                {tech.name}
              </p>
            </li>
          ))}
